...
import { type ExportSpecifier, parse as parseESM, init } from 'es-module-lexer';
...
export default function integration(
    { exportNames }: Options,
    callback: (allMataData: Map<string, Map<string, any>>) => void
): AstroIntegration {
                    ...
                    for (let page of pages) {
                        ...
                        switch (getExt(page)) {
                            ...
                            case 'ts': {
                                ...
                                const { code: js } = await esbuild.transform(
                                    pageContent,
                                    {
                                        loader: 'ts',
                                    }
...
function extractExportedMetadata(js: string, page: string, exportName: string) {
    ...
    for (let _export of exports) {
        ...
        if (typeof exportedValueString === 'undefined') {
            return;
        }
